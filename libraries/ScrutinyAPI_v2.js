			...
			for(let i in elems){
				...
				if((data.type||"text")=="text"){
					...
					for(let ii in data.items){
						elements["elem"+i+books[a]+ii] = {type:"slot", bitmap:data.bitmap||"_default_slot_empty", x: start_x+((data.size||30)*ii), y: y, size: data.size||30, visual: true}
...
let timeout = function(func, ticks, obj){
	obj = obj || {};
	Threading.initThread("timeout", function(){
		java.lang.Thread.sleep(ticks*50);
		func(obj);
	});
	/*obj = obj || {}
    var upd = {
...
Translation.addTranslation("Studied {name}", {
	ru: "Изучено {name}",
})
Network.addClientPacket("aw.achievement.give", function(scrutiny){
	Achievement.give(scrutiny.name, TranslationLoad.get("Studied {name}", [["name", scrutiny.name]]), scrutiny.icon)
});
let ScrutinyAPI = {
	save: true,
	register(name, obj){
